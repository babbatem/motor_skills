...
def main():
    ...
    img_dir = "2021_04_22_15_09_08"
    vid_path = result_path + '/' + img_dir + '/videos'
    img_path = result_path + '/' + img_dir

    if not os.path.exists(vid_path):
        os.mkdir(vid_path)
    family_dirs = os.listdir(img_path)
    family_dirs.remove('videos')

    for family_id in family_dirs:

        img_fam_path = img_path + '/' + family_id + '/'

        image_filenames = os.listdir(img_fam_path)
        ...
        print("Found", [(key, len(parsed_filenames_by_var[key])) for key in dimensions_to_vary], "images in", img_fam_path)
        if len(dimensions_to_vary) > 1:
            for dim in dimensions_to_vary:
                video = imageio.get_writer(vid_path + '/fam_' + family_id + '_v' + str(dim) + '.mp4', format='FFMPEG', mode='I', fps=15,
                               codec='h264_vaapi',
                ...
                for other_dim_val_combination in combinations_of_other_dim_vals:
                    for varied_val in dim_vals_to_vary:
                        ...
                        latest_image = imageio.imread(img_fam_path + '/' + image_filename)
            ...
            video = imageio.get_writer(vid_path + '/fam_' + family_id + '_v' + str(dim) + '.mp4', format='FFMPEG', mode='I', fps=15,
                               ...
                               pixelformat='vaapi_vld')
            parsed_filenames = parsed_filenames_by_var[dim]
            parsed_filenames = [[pf[0], [v[1] for v in pf[2]]] for pf in parsed_filenames]
            parsed_filenames.sort(key = lambda x: (x[1][2], x[1][1], x[1][0]))
            for img_fileinfo in parsed_filenames:
                latest_image = imageio.imread(img_fam_path + '/' + img_fileinfo[0])
                video.append_data(latest_image)
            video.close()
